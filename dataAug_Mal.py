...
path_label = "C:/Users/EHO085/Desktop/data_skin/ISIC-2017_Training_Part3_GroundTruth.csv"

count = 5

train_csv = np.genfromtxt(path_label, delimiter=",", dtype=None)
train_l = train_csv[1:,0]
train_mal = train_csv[1:,1]
print("Dataset length:", len(train_l))

train_label = []
for label, mal in zip(train_l, train_mal):
    if int(float(mal)) == 1:
