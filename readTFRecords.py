...
def readTFR(samp, w, h , d):
	...
	#
		...
		images, labels, names = tf.train.shuffle_batch([image, label, name], batch_size=1, capacity=samp, num_threads=1, min_after_dequeue=1)
		...
		for batch_index in range(samp):
		    img, lbl, nam = sess.run([images, labels, names])
		    # print(batch_index)
		    img = img.astype(np.uint8)
		    img_final.append(img)
		    lbl_final.append(lbl)
		    nam_final.append(nam)
	...
	lbl_final = np.array(lbl_final)

	img_final = img_final.reshape(img_final.shape[0], img_w, img_h, img_d)

	# print(img_final.shape)
	# print(lbl_final.shape)
	# print(len(nam_final))
	return (img_final, lbl_final, nam_final)
